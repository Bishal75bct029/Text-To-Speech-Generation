...
import { useAudio } from '@/app/providers/AudioProviders';
import { Button } from '@/components/ui/button';

const ProfileCard = ({ podcastData, imageUrl, userFirstName }: ProfileCardProps) => {
  const { setAudio } = useAudio();

  const [randomPodcast, setRandomPodcast] = useState<PodcastProps>();
  ...
  return (
    <div className="mt-6 flex flex-col gap-6 max-md:items-center md:flex-row">
      <Image src={imageUrl} width={250} height={250} alt="Podcaster" className="aspect-square rounded-lg" />
      <div className="flex flex-col justify-center max-md:items-center">
        ...
        {podcastData?.podcasts.length > 0 && (
          <Button onClick={playRandomPodcast}>
